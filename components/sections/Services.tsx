...
export function Services() {
  ...
  const services = [
    {
      ...
      description:
        "Свадьбы, венчания, дни рождения, крещения и другие личные события. Каждый проект создаётся индивидуально, под конкретный запрос.",
    },
    {
      icon: Briefcase,
  ...
  return (
    <section className="relative min-h-screen w-full overflow-hidden py-2.5 shadow-xl sm:py-20 lg:py-24">
      {/* Градиент-оверлей */}
          ...
          {services.map((service) => {
            ...
            return (
              <Card
                ...
                className={`group bg-pattern-shadow/30 border-pattern-accent/10 hover:bg-pattern-shadow/50 hover:border-pattern-accent/30 hover:shadow-pattern-accent/10 relative cursor-pointer transition-all duration-300 hover:shadow-xl ${service.wide ? "md:col-span-2 lg:col-span-2" : ""} ${isSelected ? "bg-pattern-shadow/50 border-pattern-accent/30 shadow-pattern-accent/10 shadow-xl" : ""} `}
              >
                <CardContent className="flex h-full min-h-[280px] flex-col justify-between p-8 sm:p-10">
                  {/* Стрелка в правом верхнем углу */}
                  {/* Иконка и название */}
                  <div className="space-y-6">
                    <div className="h-12 w-12 sm:h-14 sm:w-14">
                      <Icon
                        className="text-pattern-accent h-full w-full transition-transform duration-300 group-hover:scale-110"
                        strokeWidth={1.5}
                      />
                    </div>
                    ...
                    <div
                      className={`overflow-hidden transition-all duration-300 ease-in-out ${isSelected ? "max-h-96 opacity-100" : "max-h-0 opacity-0"} `}
                    >
                      <p className="text-pattern-light/80 pt-2 text-base leading-relaxed sm:text-lg">
                        {service.description}
                      </p>
                    </div>
                  </div>
